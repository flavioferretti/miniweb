...
route_match(const char *method, const char *path)
{
    ...
    /* Special case: /api/man* routes */
    if (strcmp(method, "GET") == 0 && strncmp(path, "/api/man", 8) == 0) {
        return man_api_handler;
    }

    /* Special case: /man/\* render routes */
    if (strcmp(method, "GET") == 0 && strncmp(path, "/man/", 5) == 0) {
...
/* Route registration */
static void
register_route(const char *method, const char *path,
               route_handler_t handler, void *handler_cls)
{
             ...
             size_t *upload_data_size, void **con_cls)
{
    ...
    if (template_render_with_data(&data, &output) != 0) {
        ...
        if (template_render("info.html", &output) != 0) {
            const char *error = "Internal Server Error: Failed to render template";
...
int
man_handler(void *cls, struct MHD_Connection *connection,
            const char *url, const char *method,
            const char *version, const char *upload_data,
            size_t *upload_data_size, void **con_cls)
{
...
int
favicon_handler(void *cls, struct MHD_Connection *connection,
                const char *url, const char *method,
                const char *version, const char *upload_data,
                size_t *upload_data_size, void **con_cls)
{
...
/* Static file handler - ONLY serves from /static/ URLs */
int
static_handler(void *cls, struct MHD_Connection *connection,
               const char *url, const char *method,
               const char *version, const char *upload_data,
               size_t *upload_data_size, void **con_cls)
{
    ...
    snprintf(path, sizeof(path), "static%s", url + 7);  /* +7 per saltare "/static" */
    ...
    if (size < 0) {
        fclose(f);
        const char *error = "Internal Server Error: Cannot get file size";
        response = MHD_create_response_from_buffer(
            strlen(error), (void *)error, MHD_RESPMEM_PERSISTENT);
        MHD_add_response_header(response, "Content-Type", "text/plain");
        ret = MHD_queue_response(connection, MHD_HTTP_INTERNAL_SERVER_ERROR, response);
    ...
    if (ext) {
        if (strcmp(ext, ".html") == 0 || strcmp(ext, ".htm") == 0) {
            ...
            MHD_add_response_header(response, "Content-Type", "application/gzip");
        }
    } else {
