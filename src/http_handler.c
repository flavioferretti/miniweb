...
/* Set response body */
void
http_response_set_body(http_response_t *resp, char *body, size_t len, int must_free)
{
	resp->body = body;
	resp->body_len = len;
...
/* Add response header */
void
http_response_add_header(http_response_t *resp, const char *name, const char *value)
{
...
http_response_free(http_response_t *resp)
{
	...
	/* Se il flag free_body è 1, dobbiamo liberare il buffer allocato (es. da fread) */
...
http_request_get_client_ip(http_request_t *req)
{
	...
	if (forwarded && forwarded[0]) {
		...
		size_t len = comma ? (size_t)(comma - forwarded)
		: strlen(forwarded);
	...
	inet_ntop(AF_INET, &req->client_addr->sin_addr,
			  req->ip_scratch, sizeof(req->ip_scratch));
	return req->ip_scratch;
}
...
http_send_json(http_request_t *req, const char *json)
{
	...
	if (write_all(req->fd, header, header_len) < 0) return -1;
	if (write_all(req->fd, json, json_len) < 0) return -1;
	return 0;
}
...
http_send_html(http_request_t *req, const char *html)
{
	...
	if (write_all(req->fd, header, header_len) < 0) return -1;
	if (write_all(req->fd, html, html_len) < 0) return -1;
	return 0;
}


/* Versione corretta di http_send_file */
int http_send_file(http_request_t *req, const char *path, const char *mime)
{
...
/* In http_handler.c */
int
http_render_template(http_request_t *req, struct template_data *data, const char *fallback_template)
{
	char *output = NULL;

	/* Tenta il rendering con i dati */
	if (template_render_with_data(data, &output) != 0) {
		/* Se fallisce e abbiamo un contenuto di pagina, prova il rendering semplice */
		if (data->page_content && template_render(data->page_content, &output) != 0) {
			/* Se fallisce tutto, usa il fallback o manda 500 */
			return http_send_error(req, 500, fallback_template ? fallback_template : "Template rendering failed");
		}
	}
	...
	/* Il parametro '1' indica a http_response_set_body di liberare la memoria di 'output' */
