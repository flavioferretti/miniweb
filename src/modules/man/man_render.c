		  ...
		  size_t abs_len)
{
	...
	if (abs_path && abs_len > 0) {
		int n =
		    snprintf(abs_path, abs_len, "%s/man/%s/%s/%s.%s",
		...
		const char *format, size_t *out_len)
{
	...
	if (!output && strcmp(format, "md") == 0) {
		if (sem_wait(&g_mandoc_semaphore) == 0) {
			char *const argv_ascii[] = {"mandoc", "-T", "ascii",
						    filepath, NULL};
			output = safe_popen_read_argv(
			    "/usr/bin/mandoc", argv_ascii, MAN_MAX_OUTPUT_SIZE,
			    10, out_len);
			sem_post(&g_mandoc_semaphore);
			if (output && *out_len > 0)
				man_strip_overstrike_ascii(output, out_len);
...
man_render_handler(http_request_t *req)
{
	...
	log_debug("[MAN] Processing URL: %s", req->url);
	log_debug("[MAN] Path part: %s", path_copy);
	...
	if (token) {
		...
		log_debug("[MAN] Section: %s", section);
	}

	// Parse page and format
	token = strtok_r(NULL, "/", &saveptr);
	if (token) {
		log_debug("[MAN] Raw page token: %s", token);

		// Check if the token has a format extension
		char *last_dot = strrchr(token, '.');
		if (last_dot && last_dot != token) {
			// We have a dot that's not at the beginning
			char *possible_format = last_dot + 1;

			// Check if the part after the dot is a valid format
			if (strcmp(possible_format, "html") == 0 ||
			    strcmp(possible_format, "pdf") == 0 ||
			    strcmp(possible_format, "ps") == 0 ||
			    strcmp(possible_format, "md") == 0 ||
			    strcmp(possible_format, "txt") == 0) {

				// This is a format extension
				*last_dot = '\0';
				strlcpy(format, possible_format,
					sizeof(format));
				strlcpy(page, token, sizeof(page));
				log_debug("[MAN] Found format extension: "
					  "page=%s, format=%s",
					  page, format);
			} else {
				// Not a valid format, could be part of the page
				// name or a section Check if it's a valid
				// section (like .5)
				if (man_is_valid_section(possible_format)) {
					// This is actually the section in the
					// filename (e.g., accept.2) Override
					// the section from URL
					*last_dot = '\0';
					strlcpy(section, possible_format,
						sizeof(section));
					strlcpy(page, token, sizeof(page));
					strlcpy(format, "html", sizeof(format));
					log_debug(
					    "[MAN] Found section in filename: "
					    "page=%s, section=%s",
					    page, section);
				} else {
					// Just a dot in the page name
					strlcpy(page, token, sizeof(page));
					strlcpy(format, "html", sizeof(format));
					log_debug(
					    "[MAN] Dot in page name: page=%s",
					    page);
				}
			}
		} else {
			...
			strlcpy(page, token, sizeof(page));
			strlcpy(format, "html", sizeof(format));
			log_debug("[MAN] No dots in filename: page=%s", page);
		}
	}

	if (page[0] == '\0' || section[0] == '\0') {
		log_debug("[MAN] Missing section or page name: section='%s', "
			  "page='%s'",
			  section, page);
	...
	if (have_paths) {
		log_debug("[MAN] Cache paths: rel=%s, abs=%s", cache_rel,
			  cache_abs);
	}
	...
	response_body =
	    man_render_cache_get(area, section, page, format, &response_len);
	if (response_body) {
		log_debug("[MAN] Cache hit");
		goto send_response;
	}
	log_debug("[MAN] Cache miss");
	...
	log_debug("[MAN] Rendering page with man_render_page(area=%s, "
	...
	if (!response_body) {
		log_debug("[MAN] man_render_page failed");
		return http_send_error(req, 404, "Manual page not found");
	}
	log_debug("[MAN] Rendering successful, got %zu bytes", response_len);

	man_render_cache_put(area, section, page, format, response_body,
			     response_len);
			...
			*last_slash = '\0';
			if (mkdir_p(cache_dir, config_static_dir) == 0)
				(void)write_file_binary(
				    cache_abs, response_body, response_len);
		}
	}
...
man_api_handler(http_request_t *req)
{
	...
	if (man_path_matches_endpoint(path, "/sections")) {
		...
		char section[16] = {0};
		char area[16] = "system";
		if (man_get_query_value(req->url, "section", section,
					sizeof(section))) {
			(void)man_get_query_value(req->url, "area", area,
						  sizeof(area));
		...
		(void)man_get_query_value(req->url, "name", name_buf,
					  sizeof(name_buf));
		(void)man_get_query_value(req->url, "section", section_buf,
					  sizeof(section_buf));

		if (name_buf[0] == '\0' || !man_is_valid_token(name_buf) ||
		    (section_buf[0] != '\0' &&
		     !man_is_valid_section(section_buf))) {
			json =
			...
			if (section_buf[0] != '\0') {
				filepath =
				    man_resolve_path(name_buf, section_buf);
			} else {
				static const char *probe_sections[] = {
				    "1", "8", "2", "3", "5",
				    "7", "6", "4", "9", "3p"};
			...
			if (!filepath || filepath[0] == '\0') {
				...
				if (!man_parse_section_from_filename(
		...
		if (path[7] == '/') {
			query = path + 8;
		} else if (man_get_query_value(req->url, "q", query_buf,
					       sizeof(query_buf))) {
			query = query_buf;
		}
