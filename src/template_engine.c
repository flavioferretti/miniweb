            ...
            const char *extra_js)
{
    ...
    temp1 = replace_single(result, "{{extra_js}}", extra_js ? extra_js : "");
    if (!temp1) {
        free(result);
        return NULL;
    }
...
template_render_with_data(struct template_data *data, char **output)
{
    ...
    if (data->extra_head_file) {
        if (read_template_file(data->extra_head_file, &extra_head) != 0) {
    ...
    result = replace_all(base_template, data->title, page_content,
                         extra_head ? extra_head : "",
                         extra_js ? extra_js : "");
    if (!result) {
        goto cleanup;
